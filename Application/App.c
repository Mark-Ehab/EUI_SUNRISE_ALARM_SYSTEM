...
char digitsArray[MAXIMUM_ALARMS];
/*Global flag that indicates which alarm is fired right now*/
volatile uint8 g_NowFiredAlarm;

Alarm_config alarms[MAXIMUM_ALARMS] = { { 0, { ALARM_INITIAL_VALUE }, 0 }, /* alarm 0 */
                                       { 0, { ALARM_INITIAL_VALUE }, 0 }, /* alarm 1 */
                                       { 0, { ALARM_INITIAL_VALUE }, 0 }, /* alarm 2 */
                                       { 0, { ALARM_INITIAL_VALUE }, 0 } /* alarm 3 */
};
...
uint8 App_setAlarm()
{
    ...
    char tempalarmdigits[MAXIMUM_ALARMS];
    ...
    while (exit_Flag == FALSE)
    {
        ...
        for (alarmindex = 0; alarmindex < MAXIMUM_ALARMS; alarmindex++)
        {
        ...
        for (alarmindex = 0; alarmindex < MAXIMUM_ALARMS; alarmindex++)
        {
            ...
            if (alarmindex % 2 == 0)
            {
                do
                {
                    ...
                    if (alarmindex == 2)
                        RTE_LCD_moveCursor(LCD_ROW_2, LCD_COLUMN_10 - alarmindex - 1);
                    /* Moving cursor to the appropriate position*/
                    else
                        RTE_LCD_moveCursor(LCD_ROW_2, LCD_COLUMN_10 - alarmindex);
                }
...
STATUS App_cancelAlarm()
{
...

    {
        if (alarms[alarmindex].flag == TRUE)
        {
            ...
            RTE_LCD_moveCursor(alarmindex, LCD_COLUMN_0);
            RTE_LCD_displayString("Alarm");
            RTE_LCD_moveCursor(alarmindex, LCD_COLUMN_6);
            RTE_LCD_integerToString(alarmindex);
            RTE_LCD_moveCursor(alarmindex, LCD_COLUMN_8);
            RTE_LCD_displayString("-> ");
            RTE_LCD_moveCursor(alarmindex, LCD_COLUMN_13);

            RTE_LCD_integerToString(alarms[alarmindex].alarmDigits[3]);
            RTE_LCD_integerToString(alarms[alarmindex].alarmDigits[2]);
            RTE_LCD_displayCharacter(':');
            /*LCD display "  :  " in coordinates in the middle of the screen*/
            RTE_LCD_moveCursor(alarmindex, LCD_COLUMN_16);
    ...
    while ((keypadBuffer > MAXIMUM_ALARMS) || (keypadBuffer < 0));
...
void ISRcallBackFunction(void)
{
    ...
    for (uint8 alarmindex = 0; alarmindex < MAXIMUM_ALARMS; alarmindex++)
    {
...
STATUS WakeUpCalculator()
{
    ...
    do
    {
        ...
        {
            keyPadBuffer = RTE_KEYPAD_getPressedKey();
            RTE_LCD_moveCursor(LCD_ROW_2, 17 + digits);
            RTE_LCD_integerToString(keyPadBuffer);
            _delay_ms(80);
...
STATUS App_listAlarm()
{
    ...
    while ((keypadBuffer > '#') || (keypadBuffer < '#'));
...
void App_welcomeScreen()
{
    ...
    LCD_clearScreen();
    LCD_moveCursor(LCD_ROW_1, LCD_COLUMN_1);
